...
// ============================================================
//  COMPTES UTILISATEURS (vos données)
// ============================================================
const USERS = [
  // Administrateurs
...
const STUDENTS = [
  ...
  { id: 's2', name: 'Lydia Ould', email: 'lydia.o@hotmail.fr', courseId: 'c1' },
  { id: 's3', name: 'Sofiane Ferhat', email: 'sofiane.f@yahoo.fr', courseId: 'c1' },
  { id: 's4', name: 'Nadia Meziani', email: 'nadia.m@gmail.com', courseId: 'c12' },
  { id: 's5', name: 'Kahina Azem', email: 'kahina.a@gmail.com', courseId: 'c2' },
  { id: 's6', name: 'Idir Sadi', email: 'idir.s@gmail.com', courseId: 'c5' }
];
...
async function initDB() {
  ...
  for (const s of STUDENTS) {
    db.run('INSERT INTO students (id, name, email, courseId) VALUES (?, ?, ?, ?)',
      [s.id, s.name, s.email, s.courseId]);
  }
  ...
  console.log('✅ Base de données Timsirin initialisée avec succès !');
  console.log('👑 Admins:', USERS.filter(u => u.role === 'admin').map(u => u.email).join(', '));
  console.log('👨‍🏫 Enseignants:', USERS.filter(u => u.role === 'teacher').map(u => u.email).join(', '));
